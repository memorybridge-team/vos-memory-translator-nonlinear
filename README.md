# SAM 2.1 Tiny → Base+ Nonlinear Memory Handoff

이 저장소는 비디오를 처리하던 SAM 2.1 Tiny의 객체별 메모리를 nonlinear translator로 변환해, SAM 2.1 Base+가 과거 영상을 전부 다시 처리하지 않고 다음 프레임부터 분할을 이어갈 수 있는지 연구합니다.
...
이 저장소는 이전 프로젝트에서 검증한 상태 추출·주입, DAVIS 평가, 기존 baseline runner, MLP 학습 코드를 **새 Git 이력으로 선별 이관한 시작점**입니다. 이전 Tiny→Large 실험은 [과거 결과](reports/legacy/)로 보존했습니다. Tiny→Base+의 checkpoint 기반 round-trip, baseline 비교, 학습 결과는 아직 생성되지 않았습니다.
...
```text
Tiny가 switch 시점 t까지 처리 → 객체별 source state 추출
                                 ↓
...
[4주 실험 계획](docs/experimental_plan.md)에 각 주의 산출물·중단 기준·평가 규칙이 있습니다. [PROJECT_CONTEXT.md](PROJECT_CONTEXT.md)는 새 작업자가 검증된 연구 문맥을 빠르게 파악하는 요약본입니다.
...
```bash
git clone https://github.com/memorybridge-team/vos-memory-translator-nonlinear-v2.git
cd vos-memory-translator-nonlinear-v2
python -m pip install -e ".[dev]"
python -m pytest -q
```

GPU Pod에서는 `bash scripts/runpod_bootstrap.sh "$PWD"`가 공식 SAM 2 및 Tiny/Base+ checkpoint를 준비합니다. 다운로드 파일의 출처와 hash를 실험 기록에 남깁니다. DAVIS/MOSEv2/LVOS v2 원본, checkpoint, 개인 SSH 키, raw state cache는 Git에 넣지 않습니다. 각 데이터셋의 이용 조건과 저장 경로를 확인한 후 수집합니다.

현재 `prepare_paired_state_dataset.py`와 일부 baseline script는 DAVIS 전용입니다. MOSEv2/LVOS v2 로더와 새로운 객체별 anchor baseline은 계획에 포함됐지만 아직 구현되지 않았습니다. 과거 Tiny→Large 명령의 target 인자를 Base+로 바꿀 때에도 checkpoint·config·cache metadata를 함께 검증해야 합니다.
